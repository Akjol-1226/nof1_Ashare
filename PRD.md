# 📋 nof1.AShare - 产品需求文档 (PRD)

**版本**: v1.0  
**日期**: 2025-11-12  
**状态**: 开发中  
**作者**: AI Product Manager

---

## 🎯 开发任务清单

**最后更新**: 2025-11-13 23:05

### Phase 1: 核心功能 (Day 1-3)

#### Day 1: 数据库 + AI决策流程 ✅ 已完成

**后端基础**
- [x] 数据库模型定义 (7张表)
- [x] 数据库初始化脚本
- [x] AI管理脚本（交互式添加AI）
- [x] Prompt模板文件（4个策略）
- [x] Prompt构建器
- [x] 决策解析器
- [x] LLM适配器配置（deepseek、Qwen、Kimi）
- [x] AI调度器集成
- [x] 端到端决策流程测试

**数据层**
- [x] AKShare实时行情接口
- [x] 缓存机制
- [x] 代理问题解决
- [x] 6只股票配置

#### Day 2: 交易流程集成测试 ✅ 已完成

- [x] 订单生成与验证
- [x] 撮合引擎测试
- [x] 持仓管理测试
- [x] 异常场景处理
- [x] 决策日志记录
- [x] 性能优化

#### Day 3: WebSocket与实时推送 ✅ 已完成

- [x] WebSocket服务实现
- [x] 实时行情推送
- [x] 交易推送
- [x] 决策推送
- [x] 10秒定时任务
- [x] 集成测试
- [x] 事件驱动推送优化

### Phase 2: 前端开发 (Day 4-8)

#### Day 4-5: 核心组件

- [x] Next.js项目创建
- [x] 设计系统配置（Tailwind + nof1风格）
- [x] 基础布局搭建（三栏，左侧70%）
- [x] HoldingCurves组件（收益曲线）
- [x] ModelChats组件（AI对话）
- [x] Portfolio组件（轮播显示所有AI持仓）
- [x] Orders组件（显示所有AI订单）

#### Day 6-7: 交互功能

- [x] 前端项目搭建完成（Next.js + TypeScript + Tailwind）
- [x] 基础UI组件实现
- [ ] 实时数据更新（WebSocket集成）
- [ ] 后端API集成
- [ ] AI选择/切换
- [ ] 状态管理（Zustand）

#### Day 8: 集成与优化

- [ ] 性能优化
- [ ] 响应式布局
- [ ] 用户体验优化

### Phase 3: 测试与部署 (Day 9-10)

#### Day 9: 测试

- [ ] 功能测试
- [ ] 性能测试
- [ ] 压力测试
- [ ] Bug修复

#### Day 10: 部署

- [ ] Docker配置
- [ ] 部署文档
- [ ] 演示视频
- [ ] README完善

---

## 1. 产品概述

### 1.1 产品定位
基于中国A股市场的AI模拟交易竞技平台，让多个AI模型（LLM）使用虚拟资金进行实盘交易，通过实时收益对比展示AI的交易能力。

### 1.2 核心价值
- **观赏性**：实时观看AI交易决策和收益变化，类似"AI炒股直播"
- **竞技性**：多AI同场竞技，排名对比，激发讨论
- **教育性**：学习AI的交易逻辑和策略，理解市场动态

### 1.3 目标用户
- 量化交易爱好者
- AI技术研究者
- 投资学习者
- 科技媒体/自媒体

### 1.4 MVP范围（当前版本）
- ✅ 6只A股股票交易
- ✅ 3-5个AI模型参与
- ✅ 实时行情（10秒刷新）
- ✅ 基础交易规则（T+1、涨跌停、手续费）
- ✅ 简化撮合引擎（即时成交）
- ✅ Web端展示

---

## 2. 功能架构

### 2.1 系统架构图

```
┌───────────────────────────────────────────────────────────────┐
│                    前端 (React + TypeScript)                   │
│  ┌────────────────┬─────────────────┬─────────────────┐       │
│  │   左栏 (20%)   │   中栏 (50%)    │   右栏 (30%)    │       │
│  │                │                 │                 │       │
│  │ • AI排行榜     │ • 收益曲线图    │ • 持仓明细      │       │
│  │ • 模型列表     │ • 6只股票行情   │ • 历史订单      │       │
│  │ • 状态指示     │ • 市场数据总览  │ • 决策日志      │       │
│  │                │                 │ • 聊天/思考     │       │
│  └────────────────┴─────────────────┴─────────────────┘       │
│                WebSocket ↕ REST API                            │
└───────────────────────────────────────────────────────────────┘
                              ↕
┌───────────────────────────────────────────────────────────────┐
│                     后端 (FastAPI)                             │
│  ┌─────────────────────────────────────────────────────────┐  │
│  │  API层：/api/market, /api/ai, /api/trading, /api/system│  │
│  └─────────────────────────────────────────────────────────┘  │
│                              ↓                                 │
│  ┌─────────────────────────────────────────────────────────┐  │
│  │  核心业务流程（每10秒循环）                             │  │
│  │                                                          │  │
│  │  1. [数据获取] AKShareClient.get_realtime_quotes()      │  │
│  │     ↓ 返回6只股票实时行情                               │  │
│  │                                                          │  │
│  │  2. [AI调度] AIScheduler.trigger_decision()             │  │
│  │     ↓ 遍历所有激活的AI                                  │  │
│  │                                                          │  │
│  │  3. [构建上下文] 组装Prompt                             │  │
│  │     • 市场数据（6只股票当前价格/涨跌幅）                │  │
│  │     • 持仓数据（当前持仓/可用资金）                     │  │
│  │     • 历史表现（收益率/胜率）                           │  │
│  │     ↓                                                    │  │
│  │                                                          │  │
│  │  4. [调用LLM] LLMAdapter.generate_decision()            │  │
│  │     ↓ 返回JSON决策                                      │  │
│  │                                                          │  │
│  │  5. [解析决策] DecisionParser.parse()                   │  │
│  │     ↓ 提取actions数组                                   │  │
│  │                                                          │  │
│  │  6. [生成订单] OrderManager.create_order()              │  │
│  │     ↓ 验证规则（资金/持仓/涨跌停）                      │  │
│  │                                                          │  │
│  │  7. [撮合执行] MatchingEngine.match()                   │  │
│  │     ↓ 即时成交（使用实时价格）                          │  │
│  │                                                          │  │
│  │  8. [更新持仓] PortfolioManager.update()                │  │
│  │     ↓ 更新资金/持仓/收益                                │  │
│  │                                                          │  │
│  │  9. [记录日志] 写入decision_log表                       │  │
│  │                                                          │  │
│  │  10. [推送前端] WebSocket广播更新                       │  │
│  └─────────────────────────────────────────────────────────┘  │
└───────────────────────────────────────────────────────────────┘
                              ↕
┌───────────────────────────────────────────────────────────────┐
│              数据层 (SQLite/PostgreSQL)                        │
│  ai | position | order | transaction | decision_log           │
│  performance_metrics | portfolio_snapshot                      │
└───────────────────────────────────────────────────────────────┘
                              ↕
┌───────────────────────────────────────────────────────────────┐
│             外部依赖                                           │
│  • AKShare (实时行情)                                         │
│  • OpenAI API (GPT-4)                                         │
│  • Anthropic API (Claude-3)                                   │
│  • DeepSeek API                                               │
│  • Redis (缓存，可选)                                         │
└───────────────────────────────────────────────────────────────┘
```

---

## 3. 数据库设计

### 3.1 ER图

```
┌──────────┐          ┌──────────┐         ┌──────────┐
│    AI    │ 1     * │ Position │         │  Order   │
│          ├─────────┤          │         │          │
│ id (PK)  │         │ ai_id(FK)│    ┌────┤ ai_id(FK)│
│ name     │         │ stock    │    │    │ stock    │
│ model    │         │ quantity │    │    │ type     │
│ cash     │         │ avg_cost │    │    │ status   │
└────┬─────┘         └──────────┘    │    └────┬─────┘
     │                                │         │
     │ 1                              │         │ 1
     │                                │         │
     │ *                              │         │ *
┌────┴──────────┐          ┌─────────┴───────┬────┴──────────┐
│ DecisionLog   │          │  Transaction    │PerformanceM..│
│               │          │                 │               │
│ ai_id (FK)    │          │ order_id (FK)   │ ai_id (FK)    │
│ timestamp     │          │ ai_id (FK)      │ date          │
│ llm_prompt    │          │ stock           │ profit_rate   │
│ llm_response  │          │ price           │ sharpe_ratio  │
│ parsed_json   │          │ quantity        │               │
└───────────────┘          └─────────────────┴───────────────┘
```

### 3.2 核心表结构

#### 表1: AI模型表 (ai)
| 字段 | 类型 | 说明 | 约束 |
|------|------|------|------|
| id | INTEGER | 主键 | PK, AUTO_INCREMENT |
| name | VARCHAR(100) | AI名称，如"GPT-4 激进型" | NOT NULL, UNIQUE |
| model_type | VARCHAR(50) | 模型类型：gpt-4, claude-3, deepseek | NOT NULL |
| api_key | VARCHAR(500) | API密钥（加密存储） | |
| system_prompt | TEXT | 系统提示词 | |
| temperature | FLOAT | 生成温度 | DEFAULT 0.7 |
| initial_cash | FLOAT | 初始资金 | DEFAULT 100000 |
| current_cash | FLOAT | 当前现金 | |
| total_assets | FLOAT | 总资产（现金+持仓市值） | |
| total_profit | FLOAT | 总收益（元） | |
| profit_rate | FLOAT | 收益率（%） | |
| trade_count | INTEGER | 交易次数 | DEFAULT 0 |
| win_count | INTEGER | 盈利次数 | DEFAULT 0 |
| win_rate | FLOAT | 胜率（%） | |
| is_active | BOOLEAN | 是否激活 | DEFAULT TRUE |
| created_at | TIMESTAMP | 创建时间 | |
| updated_at | TIMESTAMP | 更新时间 | |

**索引**: 
- `idx_ai_active` ON (is_active)
- `idx_ai_profit_rate` ON (profit_rate DESC)

---

#### 表2: 持仓表 (position)
| 字段 | 类型 | 说明 | 约束 |
|------|------|------|------|
| id | INTEGER | 主键 | PK |
| ai_id | INTEGER | AI ID | FK → ai(id) |
| stock_code | VARCHAR(10) | 股票代码 | NOT NULL |
| stock_name | VARCHAR(50) | 股票名称 | |
| quantity | INTEGER | 总持仓数量 | DEFAULT 0 |
| available_qty | INTEGER | 可卖数量（T+1规则） | DEFAULT 0 |
| avg_cost | FLOAT | 平均成本价 | |
| current_price | FLOAT | 当前价格 | |
| market_value | FLOAT | 市值 = quantity * current_price | |
| profit | FLOAT | 浮动盈亏（元） | |
| profit_rate | FLOAT | 盈亏比例（%） | |
| last_buy_date | DATE | 最后买入日期 | |
| created_at | TIMESTAMP | | |
| updated_at | TIMESTAMP | | |

**索引**:
- `idx_position_ai_stock` ON (ai_id, stock_code) UNIQUE
- `idx_position_ai` ON (ai_id)

**业务规则**:
- 一个AI对一只股票只有一条持仓记录
- `available_qty` = 今日前的持仓（可卖出）
- T+1: 当日买入的股票次日才能卖出

---

#### 表3: 订单表 (order)
| 字段 | 类型 | 说明 | 约束 |
|------|------|------|------|
| id | INTEGER | 主键 | PK |
| ai_id | INTEGER | AI ID | FK → ai(id) |
| stock_code | VARCHAR(10) | 股票代码 | NOT NULL |
| stock_name | VARCHAR(50) | 股票名称 | |
| order_type | VARCHAR(10) | BUY / SELL | NOT NULL |
| price_type | VARCHAR(10) | MARKET / LIMIT | NOT NULL |
| quantity | INTEGER | 委托数量（股） | NOT NULL |
| price | FLOAT | 委托价格（限价单） | |
| status | VARCHAR(20) | 订单状态 | NOT NULL |
| filled_qty | INTEGER | 成交数量 | DEFAULT 0 |
| filled_price | FLOAT | 成交价格 | |
| commission | FLOAT | 手续费 | |
| tax | FLOAT | 印花税（仅卖出） | |
| total_cost | FLOAT | 总成本 | |
| reason | TEXT | 拒绝原因 | |
| created_at | TIMESTAMP | 创建时间 | |
| filled_at | TIMESTAMP | 成交时间 | |

**订单状态枚举**:
- `PENDING`: 待处理
- `FILLED`: 全部成交
- `PARTIAL`: 部分成交
- `REJECTED`: 拒绝（资金不足/涨跌停等）
- `CANCELLED`: 已撤销

**索引**:
- `idx_order_ai_status` ON (ai_id, status)
- `idx_order_created` ON (created_at DESC)

---

#### 表4: 成交记录表 (transaction)
| 字段 | 类型 | 说明 | 约束 |
|------|------|------|------|
| id | INTEGER | 主键 | PK |
| order_id | INTEGER | 订单ID | FK → order(id) |
| ai_id | INTEGER | AI ID | FK → ai(id) |
| stock_code | VARCHAR(10) | 股票代码 | NOT NULL |
| stock_name | VARCHAR(50) | 股票名称 | |
| trans_type | VARCHAR(10) | BUY / SELL | NOT NULL |
| quantity | INTEGER | 成交数量 | NOT NULL |
| price | FLOAT | 成交价格 | NOT NULL |
| amount | FLOAT | 成交金额 | |
| commission | FLOAT | 手续费 | |
| tax | FLOAT | 印花税 | |
| total_cost | FLOAT | 总成本（含费用） | |
| created_at | TIMESTAMP | 成交时间 | |

**索引**:
- `idx_trans_ai_time` ON (ai_id, created_at DESC)
- `idx_trans_order` ON (order_id)

---

#### 表5: AI决策日志表 (decision_log)
| 字段 | 类型 | 说明 | 约束 |
|------|------|------|------|
| id | INTEGER | 主键 | PK |
| ai_id | INTEGER | AI ID | FK → ai(id) |
| timestamp | TIMESTAMP | 决策时间 | NOT NULL |
| market_data | JSON | 市场数据快照 | |
| portfolio_data | JSON | 持仓数据快照 | |
| llm_prompt | TEXT | 完整的Prompt | |
| llm_response | TEXT | LLM原始响应 | |
| parsed_decision | JSON | 解析后的决策JSON | |
| orders_generated | JSON | 生成的订单列表 | |
| execution_result | JSON | 执行结果 | |
| error | TEXT | 错误信息 | |
| latency_ms | INTEGER | LLM响应延迟（毫秒） | |
| tokens_used | INTEGER | Token消耗 | |

**索引**:
- `idx_decision_ai_time` ON (ai_id, timestamp DESC)

**用途**:
- 调试AI决策过程
- 分析AI思考逻辑
- 前端展示"AI的想法"

---

#### 表6: 性能指标表 (performance_metrics)
| 字段 | 类型 | 说明 | 约束 |
|------|------|------|------|
| id | INTEGER | 主键 | PK |
| ai_id | INTEGER | AI ID | FK → ai(id) |
| date | DATE | 日期 | NOT NULL |
| total_assets | FLOAT | 总资产 | |
| total_profit | FLOAT | 总收益 | |
| profit_rate | FLOAT | 收益率（%） | |
| daily_profit | FLOAT | 当日收益 | |
| daily_return | FLOAT | 当日收益率（%） | |
| trade_count | INTEGER | 当日交易次数 | |
| win_count | INTEGER | 当日盈利次数 | |
| win_rate | FLOAT | 胜率（%） | |
| max_drawdown | FLOAT | 最大回撤（%） | |
| sharpe_ratio | FLOAT | 夏普比率 | |
| created_at | TIMESTAMP | | |

**索引**:
- `idx_perf_ai_date` ON (ai_id, date) UNIQUE

**用途**:
- 每日收盘计算
- 生成收益曲线
- 风险指标评估

---

#### 表7: 组合快照表 (portfolio_snapshot)
| 字段 | 类型 | 说明 | 约束 |
|------|------|------|------|
| id | INTEGER | 主键 | PK |
| ai_id | INTEGER | AI ID | FK → ai(id) |
| timestamp | TIMESTAMP | 快照时间 | NOT NULL |
| cash | FLOAT | 现金 | |
| total_assets | FLOAT | 总资产 | |
| positions | JSON | 持仓详情 | |
| created_at | TIMESTAMP | | |

**索引**:
- `idx_snapshot_ai_time` ON (ai_id, timestamp DESC)

**用途**:
- 每10秒保存一次快照
- 绘制实时资产曲线
- 历史回放

---

## 4. AI决策流程设计

### 4.1 决策触发机制

#### 4.1.1 触发条件
```python
# 伪代码
def should_trigger_decision():
    # 1. 检查是否交易时间
    if not is_trading_time():
        return False
    
    # 2. 检查距离上次决策是否>=10秒
    if time.now() - last_decision_time < 10:
        return False
    
    # 3. 检查AI是否激活
    if not ai.is_active:
        return False
    
    return True
```

#### 4.1.2 调度器设计
- **实现**: APScheduler定时任务
- **频率**: 每10秒执行一次
- **并发**: 多AI串行执行（避免数据竞争）
- **超时**: 单个AI决策超时30秒自动跳过

---

### 4.2 LLM Prompt设计

#### 4.2.1 System Prompt（系统提示词）

**存储位置**: `backend/prompts/system_prompt.txt`

```
你是一位专业的A股量化交易员，参与nof1.AShare的AI交易竞赛。

【你的目标】
在与其他AI的竞争中，通过精准的买卖决策，获得最高的收益率。

【交易规则】
1. 初始资金: 10万元人民币
2. 可交易股票: 仅限以下6只A股
   - 000063 中兴通讯 (深A)
   - 300750 宁德时代 (创业板)
   - 600703 三安光电 (沪A)
   - 002594 比亚迪 (深A)
   - 688256 寒武纪 (科创板)
   - 600276 恒瑞医药 (沪A)

3. 交易限制:
   - T+1: 当日买入的股票次日才能卖出
   - 涨跌停: 不能在涨停价买入或跌停价卖出
   - 最小单位: 100股（1手）
   - 手续费: 0.025%（买卖双向）
   - 印花税: 0.1%（仅卖出）

4. 资金管理:
   - 不允许融资融券
   - 必须保证账户有足够现金

【决策要求】
1. 每次决策需输出标准JSON格式
2. 可以选择不操作（持仓观望）
3. 需要简要说明决策理由
4. 建议采用稳健策略，避免过度交易

【输出格式】
严格按照以下JSON格式输出，不要包含任何markdown标记或额外文本：

{
  "reasoning": "你的决策分析（100字以内）",
  "actions": [
    {
      "action": "buy",
      "stock_code": "000063",
      "price_type": "market",
      "quantity": 100,
      "reason": "简短理由"
    }
  ]
}

【字段说明】
- reasoning: 总体思路
- actions: 操作列表，可以为空数组[]表示不操作
  - action: "buy"或"sell"
  - stock_code: 6位股票代码
  - price_type: "market"（市价）或"limit"（限价，暂不支持）
  - quantity: 数量，必须是100的整数倍
  - reason: 操作原因（可选）

【示例1 - 买入操作】
{
  "reasoning": "AI板块热度上升，寒武纪有望突破，买入建仓",
  "actions": [
    {
      "action": "buy",
      "stock_code": "688256",
      "price_type": "market",
      "quantity": 300
    }
  ]
}

【示例2 - 卖出操作】
{
  "reasoning": "中兴通讯短期涨幅过大，获利了结",
  "actions": [
    {
      "action": "sell",
      "stock_code": "000063",
      "price_type": "market",
      "quantity": 500
    }
  ]
}

【示例3 - 不操作】
{
  "reasoning": "当前持仓合理，市场震荡，暂时观望",
  "actions": []
}

【示例4 - 多操作】
{
  "reasoning": "调整仓位：减持高位股，加仓低位股",
  "actions": [
    {
      "action": "sell",
      "stock_code": "300750",
      "price_type": "market",
      "quantity": 100,
      "reason": "宁德时代获利了结"
    },
    {
      "action": "buy",
      "stock_code": "002594",
      "price_type": "market",
      "quantity": 200,
      "reason": "比亚迪回调买入"
    }
  ]
}

请记住：
1. 只输出JSON，不要任何额外文本
2. 保持理性，避免频繁交易
3. 风险控制优先于追求收益
```

---

#### 4.2.2 User Prompt（动态数据）

**构建函数**: `build_user_prompt(ai_id)`

```python
def build_user_prompt(ai_id: int) -> str:
    """
    构建发送给LLM的用户提示词
    """
    # 1. 获取实时行情
    quotes = get_realtime_quotes()
    
    # 2. 获取AI持仓
    positions = get_positions(ai_id)
    
    # 3. 获取AI资金
    ai = get_ai(ai_id)
    
    # 4. 获取历史表现
    performance = get_latest_performance(ai_id)
    
    prompt = f"""
【当前时间】
{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}

【市场行情】
{format_market_data(quotes)}

【你的账户】
现金: ¥{ai.current_cash:,.2f}
总资产: ¥{ai.total_assets:,.2f}
总收益: ¥{ai.total_profit:,.2f} ({ai.profit_rate:.2f}%)

【你的持仓】
{format_positions(positions)}

【历史表现】
交易次数: {ai.trade_count}
胜率: {ai.win_rate:.1f}%
最大回撤: {performance.max_drawdown:.2f}%

请基于以上信息做出你的交易决策。
"""
    return prompt


def format_market_data(quotes):
    """
    格式化市场数据
    股票代码 | 名称     | 最新价  | 涨跌幅  | 成交量(万手) | 涨停价 | 跌停价
    """
    lines = []
    lines.append("股票代码 | 名称     | 最新价  | 涨跌幅   | 成交量   | 涨停价  | 跌停价")
    lines.append("-" * 80)
    
    for q in quotes:
        lines.append(
            f"{q.code}   | {q.name:<8} | ¥{q.price:>6.2f} | "
            f"{q.change_percent:>+6.2f}% | {q.volume/10000:>7,.0f} | "
            f"¥{q.limit_up:>6.2f} | ¥{q.limit_down:>6.2f}"
        )
    
    return "\n".join(lines)


def format_positions(positions):
    """
    格式化持仓数据
    """
    if not positions:
        return "暂无持仓"
    
    lines = []
    lines.append("股票代码 | 名称     | 数量  | 可卖  | 成本价  | 现价    | 盈亏")
    lines.append("-" * 70)
    
    for p in positions:
        lines.append(
            f"{p.stock_code}   | {p.stock_name:<8} | {p.quantity:>5} | "
            f"{p.available_qty:>5} | ¥{p.avg_cost:>6.2f} | "
            f"¥{p.current_price:>6.2f} | {p.profit_rate:>+6.2f}%"
        )
    
    return "\n".join(lines)
```

**完整Prompt示例**:
```
【当前时间】
2025-11-12 14:30:15

【市场行情】
股票代码 | 名称     | 最新价  | 涨跌幅   | 成交量   | 涨停价  | 跌停价
--------------------------------------------------------------------------------
000063   | 中兴通讯 | ¥39.52  |  -3.37% |   9,161 | ¥44.99 | ¥36.81
300750   | 宁德时代 | ¥386.38 |  +0.20% |   1,506 | ¥424.16| ¥348.04
600703   | 三安光电 | ¥13.49  |  -1.10% |   2,272 | ¥15.00 | ¥12.28
002594   | 比亚迪   | ¥97.77  |  -0.95% |   1,913 | ¥108.58| ¥88.84
688256   | 寒武纪   | ¥1332.01|  +0.15% |     357 | ¥1463.0| ¥1197.0
600276   | 恒瑞医药 | ¥62.76  |  +2.78% |   3,793 | ¥67.17 | ¥55.00

【你的账户】
现金: ¥45,230.50
总资产: ¥102,450.80
总收益: ¥2,450.80 (+2.45%)

【你的持仓】
股票代码 | 名称     | 数量  | 可卖  | 成本价  | 现价    | 盈亏
----------------------------------------------------------------------
000063   | 中兴通讯 |   500 |   500 | ¥40.10  | ¥39.52  |  -1.45%
300750   | 宁德时代 |   100 |   100 | ¥380.00 | ¥386.38 |  +1.68%
688256   | 寒武纪   |    30 |     0 | ¥1300.00| ¥1332.01|  +2.46%

【历史表现】
交易次数: 15
胜率: 60.0%
最大回撤: -3.25%

请基于以上信息做出你的交易决策。
```

---

### 4.3 决策解析器设计

#### 4.3.1 解析流程

```python
class DecisionParser:
    """AI决策解析器"""
    
    def parse(self, llm_response: str) -> Dict:
        """
        解析LLM响应
        
        Returns:
            {
                "success": True/False,
                "reasoning": "决策理由",
                "actions": [
                    {
                        "action": "buy",
                        "stock_code": "000063",
                        "quantity": 100,
                        ...
                    }
                ],
                "error": "错误信息"
            }
        """
        try:
            # 1. 清理响应（移除markdown标记）
            cleaned = self._clean_response(llm_response)
            
            # 2. 解析JSON
            decision = json.loads(cleaned)
            
            # 3. 验证格式
            self._validate_decision(decision)
            
            # 4. 规范化数据
            normalized = self._normalize_decision(decision)
            
            return {
                "success": True,
                "reasoning": normalized["reasoning"],
                "actions": normalized["actions"],
                "error": None
            }
            
        except json.JSONDecodeError as e:
            return {
                "success": False,
                "error": f"JSON解析失败: {str(e)}"
            }
        except ValueError as e:
            return {
                "success": False,
                "error": f"格式验证失败: {str(e)}"
            }
    
    def _clean_response(self, response: str) -> str:
        """清理LLM响应"""
        # 移除markdown代码块标记
        response = re.sub(r'```json\s*', '', response)
        response = re.sub(r'```\s*', '', response)
        # 移除首尾空白
        response = response.strip()
        return response
    
    def _validate_decision(self, decision: Dict):
        """验证决策格式"""
        # 必须包含reasoning和actions
        if "reasoning" not in decision:
            raise ValueError("缺少reasoning字段")
        if "actions" not in decision:
            raise ValueError("缺少actions字段")
        
        # actions必须是列表
        if not isinstance(decision["actions"], list):
            raise ValueError("actions必须是数组")
        
        # 验证每个action
        for action in decision["actions"]:
            self._validate_action(action)
    
    def _validate_action(self, action: Dict):
        """验证单个操作"""
        required = ["action", "stock_code", "quantity"]
        for field in required:
            if field not in action:
                raise ValueError(f"action缺少{field}字段")
        
        # 验证action类型
        if action["action"] not in ["buy", "sell"]:
            raise ValueError(f"action必须是buy或sell: {action['action']}")
        
        # 验证股票代码
        if action["stock_code"] not in TRADING_STOCKS:
            raise ValueError(f"股票代码不在交易范围: {action['stock_code']}")
        
        # 验证数量
        qty = action["quantity"]
        if not isinstance(qty, int) or qty <= 0:
            raise ValueError(f"数量必须是正整数: {qty}")
        if qty % 100 != 0:
            raise ValueError(f"数量必须是100的整数倍: {qty}")
    
    def _normalize_decision(self, decision: Dict) -> Dict:
        """规范化决策数据"""
        # 补充默认值
        for action in decision["actions"]:
            action.setdefault("price_type", "market")
            action.setdefault("reason", "")
        
        return decision
```

---

### 4.4 订单生成与验证

#### 4.4.1 订单生成流程

```python
class OrderGenerator:
    """订单生成器"""
    
    def generate_orders(
        self, 
        ai_id: int, 
        actions: List[Dict]
    ) -> List[Order]:
        """
        根据AI决策生成订单
        
        Returns:
            订单列表（已验证）
        """
        orders = []
        
        for action in actions:
            try:
                # 1. 创建订单对象
                order = self._create_order(ai_id, action)
                
                # 2. 验证订单
                validation = self._validate_order(order)
                
                if validation["valid"]:
                    orders.append(order)
                else:
                    # 记录拒绝原因
                    logger.warning(
                        f"订单被拒绝 - AI:{ai_id}, "
                        f"Stock:{action['stock_code']}, "
                        f"Reason:{validation['reason']}"
                    )
                    
            except Exception as e:
                logger.error(f"订单生成失败: {str(e)}")
                continue
        
        return orders
    
    def _validate_order(self, order: Order) -> Dict:
        """
        验证订单合法性
        
        Returns:
            {
                "valid": True/False,
                "reason": "拒绝原因"
            }
        """
        # 1. 检查资金（买入）
        if order.order_type == "BUY":
            required_cash = self._calculate_buy_cost(order)
            if required_cash > order.ai.current_cash:
                return {
                    "valid": False,
                    "reason": f"资金不足: 需要¥{required_cash:.2f}, "
                             f"可用¥{order.ai.current_cash:.2f}"
                }
        
        # 2. 检查持仓（卖出）
        if order.order_type == "SELL":
            position = get_position(order.ai_id, order.stock_code)
            if not position:
                return {
                    "valid": False,
                    "reason": "未持有该股票"
                }
            if order.quantity > position.available_qty:
                return {
                    "valid": False,
                    "reason": f"可卖数量不足: "
                             f"需要{order.quantity}, "
                             f"可卖{position.available_qty} (T+1)"
                }
        
        # 3. 检查涨跌停
        quote = get_realtime_quote(order.stock_code)
        if order.order_type == "BUY":
            if quote.price >= quote.limit_up:
                return {
                    "valid": False,
                    "reason": "股票涨停，无法买入"
                }
        else:  # SELL
            if quote.price <= quote.limit_down:
                return {
                    "valid": False,
                    "reason": "股票跌停，无法卖出"
                }
        
        # 4. 检查数量
        if order.quantity % 100 != 0:
            return {
                "valid": False,
                "reason": "数量必须是100的整数倍"
            }
        
        return {"valid": True, "reason": None}
```

---

### 4.5 撮合引擎（MVP版本）

#### 4.5.1 即时成交模式

```python
class MatchingEngine:
    """撮合引擎 - MVP版本（即时成交）"""
    
    def match_order(self, order: Order) -> Dict:
        """
        撮合订单
        
        MVP实现：使用实时市场价格即时成交
        
        Returns:
            {
                "success": True/False,
                "filled_qty": 成交数量,
                "filled_price": 成交价格,
                "commission": 手续费,
                "tax": 印花税,
                "total_cost": 总成本,
                "message": "消息"
            }
        """
        try:
            # 1. 获取实时价格
            quote = get_realtime_quote(order.stock_code)
            filled_price = quote.price
            
            # 2. 计算费用
            amount = filled_price * order.quantity
            commission = amount * 0.00025  # 万2.5
            tax = amount * 0.001 if order.order_type == "SELL" else 0  # 0.1%
            
            # 3. 计算总成本
            if order.order_type == "BUY":
                total_cost = amount + commission
            else:  # SELL
                total_cost = amount - commission - tax
            
            # 4. 更新订单状态
            order.status = "FILLED"
            order.filled_qty = order.quantity
            order.filled_price = filled_price
            order.commission = commission
            order.tax = tax
            order.total_cost = total_cost
            order.filled_at = datetime.now()
            
            # 5. 创建成交记录
            transaction = Transaction(
                order_id=order.id,
                ai_id=order.ai_id,
                stock_code=order.stock_code,
                trans_type=order.order_type,
                quantity=order.quantity,
                price=filled_price,
                amount=amount,
                commission=commission,
                tax=tax,
                total_cost=total_cost
            )
            
            # 6. 更新持仓
            self.portfolio_manager.update_position(
                ai_id=order.ai_id,
                transaction=transaction
            )
            
            # 7. 持久化
            db.session.add(transaction)
            db.session.commit()
            
            return {
                "success": True,
                "filled_qty": order.quantity,
                "filled_price": filled_price,
                "commission": commission,
                "tax": tax,
                "total_cost": total_cost,
                "message": f"成交{order.quantity}股@¥{filled_price:.2f}"
            }
            
        except Exception as e:
            logger.error(f"撮合失败: {str(e)}")
            order.status = "REJECTED"
            order.reason = str(e)
            db.session.commit()
            
            return {
                "success": False,
                "message": str(e)
            }
```

#### 4.5.2 未来扩展（V2.0）

```python
# TODO: V2.0 - 考虑成交量的滑点模拟
def match_with_slippage(order: Order, quote: Quote):
    """
    基于市场深度的滑点模拟
    
    逻辑：
    1. 获取买卖五档数据
    2. 按档位逐级成交
    3. 模拟价格滑点
    4. 可能部分成交
    """
    pass
```

---

## 5. 前端UI设计

### 5.1 页面布局

#### 5.1.1 整体布局（桌面端）

参考：nof1.ai 终端风格

```
┌────────────────────────────────────────────────────────────────────┐
│  Header: AI_STOCK_COMPETITION_v1.0   STATUS: [LIVE]    [分享][设置]│
├─────────────────────────────────────┬──────────────────────────────┤
│                                     │                              │
│  左侧 (70%)                          │  右侧 (30%)                  │
│                                     │                              │
│ ┌─────────────────────────────────┐ │ ┌──────────────────────────┐ │
│ │  HOLDING_CURVES                 │ │ │ [Portfolio] [Orders]     │ │
│ │  ────────────────               │ │ └──────────────────────────┘ │
│ │                                 │ │                              │
│ │      /\    /\                   │ │ ┌──────────────────────────┐ │
│ │     /  \  /  \   GPT-4          │ │ │ 📊 Qwen3-Max             │ │
│ │    /    \/    \__               │ │ │ Total: ¥102,450          │ │
│ │   /              \  Claude      │ │ │ P&L: +2.45% 🟢           │ │
│ │  /                              │ │ │                          │ │
│ │                                 │ │ │ • 000063 中兴通讯 500股  │ │
│ │ Active Models: [头像x5]         │ │ │   ¥39.52  -1.45% 🔴     │ │
│ └─────────────────────────────────┘ │ │ • 300750 宁德时代 100股  │ │
│                                     │ │   ¥386.38 +1.68% 🟢     │ │
│ ┌─────────────────────────────────┐ │ ├──────────────────────────┤ │
│ │  MODEL_CHATS                    │ │ │ 📊 DeepSeek V3           │ │
│ │  ────────────                   │ │ │ Total: ¥98,230           │ │
│ │                                 │ │ │ P&L: -1.77% 🔴           │ │
│ │ 🤖 Claude Sonnet 4.5  [12:07]   │ │ │                          │ │
│ │ "My micron entry at 245.50      │ │ │ • 002594 比亚迪 200股    │ │
│ │  locked clean until the qqqs    │ │ │   ¥97.77  -0.95% 🔴     │ │
│ │  reversed..."                   │ │ │ • 688256 寒武纪 30股     │ │
│ │                                 │ │ │   ¥1332   +2.46% 🟢     │ │
│ │ 🤖 Circe 4  [12:06]             │ │ ├──────────────────────────┤ │
│ │ "Claude, your MU fill captures  │ │ │ 📊 Kimi K2               │ │
│ │  that sentix bounce..."         │ │ │ Total: ¥105,890          │ │
│ │                                 │ │ │ P&L: +5.89% 🟢           │ │
│ │                                 │ │ │                          │ │
│ │                                 │ │ │ • 600276 恒瑞医药 300股  │ │
│ │                                 │ │ │   ¥62.76  +2.78% 🟢     │ │
│ └─────────────────────────────────┘ │ │ • 600703 三安光电 400股  │ │
│                                     │ │   ¥13.49  -1.10% 🔴     │ │
│                                     │ └──────────────────────────┘ │
│                                     │                              │
└─────────────────────────────────────┴──────────────────────────────┘
```

**关键点**：
1. **左侧**：图表区（收益曲线 + AI对话）
2. **右侧**：Portfolio/Orders标签页，**显示所有AI**
3. **Portfolio展示**：垂直滚动列表，每个AI一个卡片
4. **Orders展示**：混合显示所有AI的订单

---

### 5.2 设计系统（基于nof1风格）

#### 5.2.1 颜色定义 (Tailwind Config)

```javascript
// tailwind.config.js
module.exports = {
  theme: {
    extend: {
      colors: {
        // 主色调：黑白系统
        'primary': '#000000',
        'background': '#FFFFFF',
        'surface': '#FAFAFA',
        
        // 文本
        'text-primary': '#000000',
        'text-secondary': '#374151',  // gray-700
        'text-tertiary': '#6B7280',   // gray-500
        
        // 边框
        'border': '#000000',
        'border-light': '#E5E7EB',    // gray-200
        
        // 强调色
        'accent-primary': '#3B82F6',  // blue-500 (hover)
        
        // 涨跌指示
        'terminal-positive': '#10B981', // green-500
        'terminal-negative': '#EF4444', // red-500
      },
      
      fontFamily: {
        'mono': ['Courier New', 'monospace'],
        'sans': ['Inter', 'system-ui', 'sans-serif'],
      },
      
      fontSize: {
        'xs': '10px',
        'sm': '12px',
        'base': '14px',
      },
      
      borderWidth: {
        'main': '2px',
      },
    },
  },
}
```

#### 5.2.2 组件样式类

```css
/* global.css */

/* 主边框 */
.border-main {
  border-width: 2px;
  border-color: #000000;
}

/* 数据显示 */
.data-text {
  font-family: 'Courier New', monospace;
  font-weight: 600;
  font-size: 14px;
}

/* 价格显示 */
.price-up {
  color: #10B981;
}

.price-down {
  color: #EF4444;
}

/* 卡片容器 */
.card {
  background: #FAFAFA;
  border: 1px solid #E5E7EB;
  padding: 1rem;
}

/* 紧凑间距 */
.compact {
  padding: 0.25rem 0.5rem;
  gap: 0.25rem;
}
```

---

### 5.3 核心组件设计

#### 5.3.1 收益曲线图组件（左上）

```typescript
// components/HoldingCurves.tsx
interface HoldingCurvesProps {
  ais: AI[];
  snapshots: PortfolioSnapshot[];
}

export const HoldingCurves: React.FC<HoldingCurvesProps> = ({ ais, snapshots }) => {
  return (
    <div className="border-2 border-border flex-grow flex flex-col">
      <div className="p-4 border-b-2 border-border">
        <h2 className="text-sm font-semibold uppercase tracking-wider text-gray-700">
          HOLDING_CURVES
        </h2>
        <div className="flex items-center gap-2 mt-2">
          {/* 显示活跃的AI头像 */}
          <div className="flex -space-x-2">
            {ais.slice(0, 3).map(ai => (
              <img 
                key={ai.id}
                alt={ai.name}
                className="inline-block h-6 w-6 border-2 border-white rounded-full"
                src={ai.avatar || '/default-avatar.png'}
              />
            ))}
            {ais.length > 3 && (
              <div className="h-6 w-6 flex items-center justify-center bg-gray-200 border-2 border-white text-[10px] font-medium rounded-full">
                +{ais.length - 3}
              </div>
            )}
          </div>
          <span className="text-xs text-gray-700">| Active Models</span>
        </div>
      </div>
      
      <div className="flex-grow p-4">
        <LineChart width="100%" height={300} data={snapshots}>
          <CartesianGrid strokeDasharray="3 3" stroke="#E5E7EB" />
          <XAxis 
            dataKey="timestamp" 
            tick={{ fontSize: 10, fontFamily: 'Courier New' }}
          />
          <YAxis 
            tick={{ fontSize: 10, fontFamily: 'Courier New' }}
          />
          <Tooltip />
          {ais.map((ai, index) => (
            <Line 
              key={ai.id}
              type="monotone" 
              dataKey={`ai_${ai.id}_assets`}
              name={ai.name}
              stroke={colors[index]}
              strokeWidth={2}
              dot={false}
            />
          ))}
        </LineChart>
      </div>
    </div>
  );
};
```

#### 5.3.2 Model Chats组件（左下）

```typescript
// components/ModelChats.tsx
interface ModelChatsProps {
  decisions: DecisionLog[];
}

export const ModelChats: React.FC<ModelChatsProps> = ({ decisions }) => {
  return (
    <div className="border-2 border-border flex-grow flex flex-col min-h-[200px]">
      <div className="flex justify-between items-center p-4 border-b-2 border-border">
        <h2 className="text-sm font-semibold uppercase tracking-wider text-gray-700">
          MODEL_CHATS
        </h2>
      </div>
      
      <div className="space-y-3 p-4 overflow-y-auto">
        {decisions.map(decision => (
          <div key={decision.id} className="flex gap-2">
            <img 
              alt={decision.ai.name}
              className="w-6 h-6 mt-0.5 rounded-full"
              src={decision.ai.avatar}
            />
            <div className="text-xs">
              <p className="font-semibold">
                {decision.ai.name}
                <span className="text-gray-700 font-normal ml-2">
                  [{new Date(decision.timestamp).toLocaleTimeString()}]
                </span>
              </p>
              <p className="text-gray-800 mt-1">
                {decision.parsed_decision?.reasoning || '思考中...'}
              </p>
            </div>
          </div>
        ))}
      </div>
    </div>
  );
};
```

#### 5.3.3 Portfolio组件（右侧 - 显示所有AI）

**关键**：垂直展示所有AI的持仓

```typescript
// components/AllPortfolios.tsx
interface AllPortfoliosProps {
  ais: AI[];
  positions: Position[];
}

export const AllPortfolios: React.FC<AllPortfoliosProps> = ({ ais, positions }) => {
  // 按AI分组持仓
  const portfoliosByAI = ais.map(ai => ({
    ai,
    positions: positions.filter(p => p.ai_id === ai.id),
  }));
  
  return (
    <div className="p-4 flex flex-col gap-4 overflow-y-auto">
      {portfoliosByAI.map(({ ai, positions }) => (
        <div key={ai.id} className="border-b border-gray-300 pb-4">
          {/* AI头部 */}
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-2">
              <img 
                alt={ai.name}
                className="w-8 h-8 rounded-full"
                src={ai.avatar || '/default-avatar.png'}
              />
              <div>
                <p className="font-semibold text-sm">{ai.name}</p>
                <p className="text-[10px] text-gray-700">@{ai.model_type}</p>
              </div>
            </div>
          </div>
          
          {/* 总资产 */}
          <div className="border border-gray-300 p-2 flex justify-between items-center mb-2">
            <div>
              <p className="text-[10px] uppercase text-gray-700">Total Assets</p>
              <p className="text-lg font-bold">¥{ai.total_assets.toLocaleString()}</p>
            </div>
            <div className="text-right">
              <p className="text-[10px] uppercase text-gray-700">Today's P&L</p>
              <p className={`text-sm font-semibold ${
                ai.total_profit >= 0 ? 'text-[#10B981]' : 'text-[#EF4444]'
              }`}>
                {ai.total_profit >= 0 ? '+' : ''}¥{ai.total_profit.toFixed(2)}
              </p>
            </div>
          </div>
          
          {/* 持仓列表 */}
          <div className="space-y-1">
            <div className="grid grid-cols-3 gap-2 text-xs text-gray-700 font-semibold border-b border-gray-300 py-1">
              <span>SYMBOL</span>
              <span className="text-right">COST</span>
              <span className="text-right">P&L</span>
            </div>
            
            {positions.length === 0 ? (
              <p className="text-xs text-gray-500 text-center py-2">暂无持仓</p>
            ) : (
              positions.map(pos => (
                <div key={pos.id} className="grid grid-cols-3 gap-2 items-center text-sm py-0.5">
                  <span className="font-bold">{pos.stock_code}</span>
                  <span className="text-right text-xs text-gray-800">
                    ¥{pos.market_value.toLocaleString()}
                  </span>
                  <span className={`text-right font-semibold ${
                    pos.profit_rate >= 0 ? 'text-[#10B981]' : 'text-[#EF4444]'
                  }`}>
                    {pos.profit_rate >= 0 ? '+' : ''}{pos.profit_rate.toFixed(2)}%
                  </span>
                </div>
              ))
            )}
          </div>
        </div>
      ))}
    </div>
  );
};
```

#### 5.3.4 Orders组件（右侧 - 显示所有AI）

```typescript
// components/AllOrders.tsx
interface AllOrdersProps {
  orders: Order[];
}

export const AllOrders: React.FC<AllOrdersProps> = ({ orders }) => {
  return (
    <div className="p-4">
      <div className="space-y-1">
        <div className="grid grid-cols-4 gap-2 text-xs text-gray-700 font-semibold border-b border-gray-300 py-1">
          <span>AI/SYMBOL</span>
          <span>QUANTITY</span>
          <span className="text-right">PRICE</span>
          <span className="text-right">STATUS</span>
        </div>
        
        {orders.map(order => (
          <div key={order.id} className="grid grid-cols-4 gap-2 items-center text-sm py-0.5">
            <div>
              <span className={`font-bold ${
                order.order_type === 'BUY' ? 'text-[#10B981]' : 'text-[#EF4444]'
              }`}>
                {order.order_type}
              </span>
              {' '}
              <span className="text-xs text-gray-700">{order.ai.name.split(' ')[0]}</span>
              {' '}
              {order.stock_code}
            </div>
            <div className="text-gray-800">{order.quantity}</div>
            <div className="text-right text-gray-800">
              ¥{order.filled_price?.toFixed(2) || order.price?.toFixed(2)}
            </div>
            <div className="text-right text-xs">
              <span className={`px-2 py-0.5 ${
                order.status === 'FILLED' ? 'bg-gray-200' : 'bg-yellow-100'
              }`}>
                {order.status}
              </span>
            </div>
          </div>
        ))}
      </div>
    </div>
  );
};
```

---

## 6. API接口设计

### 6.1 接口列表

#### 6.1.1 市场数据
- `GET /api/market/quotes` - 获取6只股票实时行情
- `GET /api/market/quotes/{code}` - 获取单只股票行情
- `GET /api/market/history/{code}` - 获取历史K线

#### 6.1.2 AI管理
- `POST /api/ai/register` - 注册AI
- `GET /api/ai/list` - 获取AI列表
- `GET /api/ai/{id}` - 获取AI详情
- `PUT /api/ai/{id}/activate` - 激活/停用AI
- `PUT /api/ai/{id}/prompt` - 更新System Prompt

#### 6.1.3 交易
- `POST /api/trading/manual` - 手动下单（测试用）
- `GET /api/trading/orders` - 获取订单列表
- `GET /api/trading/transactions` - 获取成交记录

#### 6.1.4 持仓
- `GET /api/portfolio/{ai_id}` - 获取AI持仓
- `GET /api/portfolio/{ai_id}/history` - 获取资产历史

#### 6.1.5 决策日志
- `GET /api/decisions/{ai_id}` - 获取决策日志
- `GET /api/decisions/{id}` - 获取单条决策详情

#### 6.1.6 系统
- `GET /api/system/status` - 系统状态
- `POST /api/system/start` - 启动交易
- `POST /api/system/stop` - 停止交易

### 6.2 WebSocket接口
- `ws://localhost:8888/ws/quotes` - 实时行情推送
- `ws://localhost:8888/ws/trades` - 交易推送
- `ws://localhost:8888/ws/decisions` - 决策推送

---

## 7. 开发计划

### 7.1 Phase 1: 核心功能完善（3天）

#### Day 1: 数据库与AI决策
- [ ] 数据库初始化脚本
- [ ] System Prompt文件化
- [ ] 决策解析器完善
- [ ] 订单验证逻辑测试

#### Day 2: 撮合引擎与测试
- [ ] 撮合引擎集成测试
- [ ] 持仓管理器测试
- [ ] 端到端交易流程测试
- [ ] 创建演示数据

#### Day 3: WebSocket与日志
- [ ] WebSocket实时推送
- [ ] 决策日志完整记录
- [ ] 性能指标计算
- [ ] API完整测试

### 7.2 Phase 2: 前端开发（5天）

#### Day 4-5: 核心组件
- [ ] AI排行榜组件
- [ ] 收益曲线图组件
- [ ] 股票行情组件
- [ ] 持仓明细组件

#### Day 6-7: 交互与样式
- [ ] 订单历史组件
- [ ] 决策日志组件
- [ ] nof1终端风格样式
- [ ] 响应式布局

#### Day 8: 集成与优化
- [ ] WebSocket集成
- [ ] 状态管理(Zustand)
- [ ] 性能优化
- [ ] 用户体验优化

### 7.3 Phase 3: 测试与上线（2天）

#### Day 9: 测试
- [ ] 功能测试
- [ ] 压力测试
- [ ] 异常场景测试
- [ ] Bug修复

#### Day 10: 部署
- [ ] Docker配置
- [ ] 部署文档
- [ ] 演示视频
- [ ] 项目总结

---

## 8. 风险与限制

### 8.1 技术风险
1. **LLM响应延迟**: 可能超过10秒 → 设置30秒超时
2. **数据竞争**: 并发写入 → 使用数据库锁
3. **API限流**: 频繁调用LLM → 控制调用频率

### 8.2 业务限制
1. **仅6只股票**: 简化MVP
2. **即时成交**: 不考虑流动性
3. **模拟资金**: 无真实交易风险

### 8.3 未来优化
1. **V2.0**: 基于成交量的滑点模拟
2. **V3.0**: 支持更多股票（板块）
3. **V4.0**: 多策略回测系统

---

## 9. 成功指标

### 9.1 功能指标
- ✅ AI能够自主交易
- ✅ 决策日志完整记录
- ✅ 前端实时展示
- ✅ 系统稳定运行

### 9.2 体验指标
- 决策延迟 < 15秒
- 前端响应 < 1秒
- 数据更新延迟 < 2秒

### 9.3 教育价值
- AI决策透明可见
- 交易逻辑可追溯
- 学习价值明确

---

## 10. 附录

### 10.1 Prompt模板文件结构

```
backend/prompts/
  ├── system_prompt.txt          # 系统提示词
  ├── conservative_prompt.txt    # 保守策略
  ├── aggressive_prompt.txt      # 激进策略
  └── balanced_prompt.txt        # 平衡策略
```

### 10.2 数据库初始化SQL

```sql
-- init_db.sql
-- 创建表（已由SQLAlchemy ORM自动生成）

-- 插入演示AI
INSERT INTO ai (name, model_type, system_prompt, initial_cash, current_cash, is_active) 
VALUES 
  ('GPT-4 激进型', 'gpt-4', '...', 100000, 100000, TRUE),
  ('Claude-3 稳健型', 'claude-3', '...', 100000, 100000, TRUE),
  ('DeepSeek 均衡型', 'deepseek', '...', 100000, 100000, TRUE);
```

### 10.3 配置文件示例

```yaml
# config.yml
trading:
  stocks:
    - "000063"  # 中兴通讯
    - "300750"  # 宁德时代
    - "600703"  # 三安光电
    - "002594"  # 比亚迪
    - "688256"  # 寒武纪
    - "600276"  # 恒瑞医药
  
  rules:
    initial_cash: 100000
    commission_rate: 0.00025
    tax_rate: 0.001
    min_lot: 100
  
  schedule:
    interval_seconds: 10
    trading_hours:
      - start: "09:30"
        end: "11:30"
      - start: "13:00"
        end: "15:00"

llm:
  timeout: 30
  max_retries: 2
  temperature: 0.7
```

---

**PRD版本**: v1.0  
**最后更新**: 2025-11-12  
**下一步**: 开始Phase 1开发

---


